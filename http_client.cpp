...
/*	s_req_callback = req_callback;
	...
	mg_connect_http* connection = mg_connect_http(&mgr, OnHttpEvent, url.c_str(), NULL, NULL);
...
void HttpClient::PostReq(const std::string &url, ReqCallback req_callback, const std::string &data)
{
	// 给回调函数赋值
	s_req_callback = req_callback;
	mg_mgr mgr;
	mg_mgr_init(&mgr, NULL);
	mg_connection* connection = mg_connect_http(&mgr, OnHttpEvent, url.c_str(), "Content-Type: application/x-www-form-urlencoded\r\n", data.c_str());
	mg_set_protocol_http_websocket(connection);
