...
bool CHttpHeader::Revolse( const std::string& strHeader )
{
	...
	do
	{
		...
		if ( 0 == nLineIndex )//第一行
		{
			strncpy(m_szHttpVersion, strLine.c_str(), 8);
			m_szHttpVersion[8]='\0';
...
bool CHttpSocket::DownloadFile( const wstring& strUrl, const wstring& strSavePath )
{
	...
	try
	{
		...
		if ( ! InitSocket(str, uPort) )
			throw L"";
		HTTP_HERDER header;
		strcpy(header.szHostName, str.c_str());
__request:
		...
		do
		{
			...
			if ( nRecvSize>0 )
			{
				...
				if ( !bFilter )
				{
					...
					nRecvSize-=(nPos+4);
					FILE* fp = NULL;
				//	_wfopen(&fp, strSavePath.c_str(), L"ab+");
					if ( NULL == fp )
							  ...
							  const char* pRange/*=NULL*/, const char* pAccept/*="* / *"*/ )
{
	...
	if ( get == type )
		strcpy(header.szType, "GET");
	else
		strcpy(header.szType, "POST");
	memset(header.szRequest, 0, MAX_PATH);
	strcpy(header.szRequest, pRequest);
	memset(header.szAccept, 0, 100);
	strcpy(header.szAccept, pAccept);
	memset(header.szRange, 0, 11);
	if ( pRange )
		strcpy(header.szRange, pRange);
}
