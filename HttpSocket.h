...
struct HTTP_HERDER
{
	...
	HTTP_HERDER()
	{
		...
		strcat(szVersion, "HTTP/1.1");
		strcat(szConnection, "Keep-Alive");
	}
	string ToString()
	{
		string strRet;
...
class SERVICE_HTTP_CLASS CHttpHeader
{
public:
...
class SERVICE_HTTP_CLASS CHttpSocket: public IHttpInterface2
{
public:
	CHttpSocket();
	~CHttpSocket();
